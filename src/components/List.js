...
const List = ({ keyword, contact, setSelectContactId, selectContactId }) => {
    return (
        <ul className="eysHZq">
            {
                contact && contact
                    .filter(item => item.name.indexOf(keyword) !== -1 || String(item.age).indexOf(keyword) !== -1 || item.phoneNumber.indexOf(keyword) !== -1 || item.email.indexOf(keyword) !== -1 || (item.description !== null && (item.description.indexOf(keyword) !== -1)))
